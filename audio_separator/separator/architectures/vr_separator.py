...
class VRSeparator(CommonSeparator):
    ...
    def separate(self, audio_file_path, custom_output_names=None):
        ...
        # VR models use PyTorch directly - STFT now handles ROCm via CPU offload
        inference_device = self.torch_device

        self.model_run.load_state_dict(torch.load(self.model_path, map_location="cpu"))
        self.model_run.to(inference_device)
        self.logger.debug("Model loaded and moved to device.")

        y_spec, v_spec = self.inference_vr(
            self.loading_mix(), inference_device, self.aggressiveness
        )
