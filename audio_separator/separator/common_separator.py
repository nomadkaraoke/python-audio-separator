...
class CommonSeparator:
    ...
    def __init__(self, config):
        ...
        self.is_rocm = config.get("is_rocm", False)
    ...
    def write_audio_pydub(self, stem_path: str, stem_source):
        ...
        try:
            ...
            self.logger.debug(
                f"Exported audio file successfully to {stem_path} with {output_bit_depth}-bit depth"
            )
        except (IOError, ValueError) as e:
    ...
    def write_audio_soundfile(self, stem_path: str, stem_source):
        ...
        try:
            ...
            self.logger.debug(
                f"Exported audio file successfully to {stem_path} with subtype {output_subtype}"
            )
        except Exception as e:
            self.logger.error(f"Error exporting audio file: {e}")
